    ...
    this.commands = {
      ...
      run: function(a) {
        ...
        return Q.resolve(cell.car.split(',')).then(function(keys) {
          return [keys, self.napi("session/new", JSON.stringify({keys: keys, interactive: !!self.config.interactive}))];
            ...
            .then(function() {
              ...
              self.ev.on("keypress", function(chunk, key) {
                ...
                else if (key)
                {
                  ...
                  {
                    aborted = true;
                    return self.napi("session/" + sid, false, "DELETE").done();
                  }
                  else if (key.name == 's')
                  {
                    return self.napi("session/" + sid)
