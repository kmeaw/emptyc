...
(function() {
    ...
    this.commands = {
      ...
      run: function(a) {
        ...
        return this.resolve(cell.car.split(',')).then(function(keys) {
          ...
          if (self.config.sudo)
            sudo = "sudo ";
          return self.napi("session/" + session.id + "/run", sudo + cell.cdr)
            .then(function() {
              var exits = {};
              var failures = {};
              self.mode = self.config("mode");

              self.ev.on("abort-session", function() {
                aborted = true;
                self.napi("session/" + session.id, false, "DELETE").done();
              });

              self.ev.on("show-status", function() {
                self.napi("session/" + session.id)
                  .then(function(s) { console.log(s); })
                  .done();
              });

              self.ev.on("write", function(chunk) {
                self.napi("session/" + session.id + "/write", chunk).done();
              });

              self.ev.on("set-mode", function(new_mode) {
                self.mode = new_mode;
                console.log("Mode has been switched to ``%s''.", self.mode);
              });

              self.ev.on("kill-child", function(key) {
                self.napi("session/" + session.id)
                  .then(function(s) {
                    var k = key;
                    if (!k)
              ...
              {
                if (emptyc.mode == "line" || emptyc.mode == "write")
                  console.log("%s: %s", key, data.trim());
                else if (emptyc.mode == "stream")
                {
  ...
  module.exports.prototype.exec = function(line) {
    ...
    if (this.aliases[cell.car])
      cell.car = this.aliases[cell.car];
  ...
  module.exports.prototype.completer = function(line, callback) {
    ...
    if (cmd)
    {
      if (this.completers[cell.car])
        this.completers[cell.car](cell.cdr, function(cdrs) { 
