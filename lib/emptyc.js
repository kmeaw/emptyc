...
(function() {
  ...
  module.exports = function Emptyc() {
    ...
    this.commands = {
      ...
      toggle: function(car) {
        /* help: <option>: toggle config option value */
      ...
      su: function(car) {
        /* help: [<user>]: switch between default and specified (or root) user */
      ...
      help: function() {
        /* help: this help */
        var maxl = 0;
        Object.keys(this.commands).forEach(function(c) {
          while (c.length > maxl)
            maxl = c.length;
        });
        for(var c in this.commands) {
          if (!c || c == "notfound") continue;
          var spaces = (new Array(maxl - c.length + 4)).join(' ');
          var cmd = this.commands[c];
          var help = new RegExp('/[*] help: ((?:[^*]|[*][^/])+) [*]/').exec(cmd.toString()) || ["",""];
          this.ev.emit("info", util.format("%s%s%s", c, spaces, help[1]));
        }
        return Q.resolve();
      },

      run: function(a) {
        /* help: <hosts> <cmd>: execute <cmd> on <hosts> (list is comma-separated) */
        var cell = argshift(a);
        var t = Date.now();
        var aborted = false;
        var self = this;
      ...
      resolve: function(car) {
        /* help: <hosts>: run internal resolver for specified expression */
        var ev = this.ev;
        if (!car) return Q.reject("resolve <hosts>");
      ...
      res.on('end', function() {
        if (res.statusCode == 200)
        {
          ...
          try {
            ...
            return deferred.resolve([null, "nop", null]); // reject("Cannot parse: " + body);
          }
