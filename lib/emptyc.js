...
var Commands = {
  ...
  run: function(a, ev) {
    ...
    return Q.resolve(cell.car.split(',')).then(function(keys) {
      return [keys, napi("session/new", JSON.stringify(keys))];
              ...
              .spread(function(key, type, data) {
                switch(type)
                {
                  ...
                  case "dead":
                    ...
                    break;
                  case "error":
                    console.warn("%s %s %s", key, "<error>".bold, data.trim().red);
                    break;
                  case "done":
                    if (key == null)
                    {
                      console.log("Run took %d ms, %d hosts have failed", Date.now() - t, Object.keys(failures).length);
